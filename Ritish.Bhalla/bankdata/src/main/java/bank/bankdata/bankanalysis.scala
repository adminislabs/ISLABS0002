...
//var averageof_CreditCard = zippedCredit.map{case (a,b) => (f"Average Transaction Amount from Credit = $b%.2f")}.foreach(println)
...
//Combining D from first and second file
var zippedDebit = zipRddDebit.zip(zipRddDebit2).map{case ((a,b),(c,d)) => (a,(b+d)/2)}
//var averageof_DebitCard = zippedDebit.map{case (a,b) => (f"Average Transaction Amount from Debit = $b%.2f")}.foreach(println)
...
//var averageof_allyear = yearCredit.map{case (a,b) =>(f"Year = $a%5s",f"Average Transaction Amount from Credit =$b%.2f")}.foreach(println)
...
//var abc = maxKey2.map{case (a,b) => (f"Account id = $a%10s",f" Balance = $b%5s")}.foreach(println)
println("")
var averageof_CreditCard = zippedCredit.map{case (a,b) => (f" Average Transaction Amount from Credit = $b%.2f")}.foreach(println)
var averageof_DebitCard = zippedDebit.map{case (a,b) => (f" Average Transaction Amount from Debit = $b%.2f")}.foreach(println)
println("")
var averageof_allyear = yearCredit.map{case (a,b) =>(f"Year = $a%5s",f" Average Transaction Amount from Credit = $b%.2f")}.foreach(println)
println("")
var averageof_allyear_debit = yearCredit_debit.map{case (a,b) =>(f"Year = $a%5s",f" Average Transaction Amount from Debit = $b%.2f")}.foreach(println)
println("")
println("Account_id whose balance is highest and its Balance = "+maxKey2)
println("")
/*println(averageof_CreditCard)
println(averageof_DebitCard)
println(averageof_allyear)
println(averageof_allyear_debit)
println(balanceRdd)*/
sc.stop()
}
